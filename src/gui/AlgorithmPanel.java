...
public class AlgorithmPanel extends JPanel implements ActionListener {
	JPanel algNumberPanel;
	JPanel algDetailsPanel;
	JPanel argDetailsPanel;
	JPanel buttonPanel;
	JPanel dataPanel;
	...
	public AlgorithmPanel() {
		...
		Border paneEdge = BorderFactory.createTitledBorder(blackline,
				"Algorithm Panel");

		algorithmPanel.setBorder(paneEdge);
		...
		layout.setHorizontalGroup(layout
				...
				.addGroup(
						layout.createParallelGroup(
								...
								.addComponent(nameTf).addComponent(filepathTf)
								.addComponent(distBox).addComponent(prefixTf)
				...
				.addGroup(
						layout.createParallelGroup(
								...
								.addComponent(prev).addComponent(next))
		...
		layout.setVerticalGroup(layout
				...
				.addGroup(
						layout.createParallelGroup(
								...
								.addComponent(number).addComponent(numberTf)
								.addComponent(total).addComponent(totalAlg)
								.addComponent(prev).addComponent(next))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(id).addComponent(algorithmIDTf))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(name).addComponent(nameTf))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(filepath)
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(dist).addComponent(distBox))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(arg).addComponent(argumentTf)
								.addComponent(argumentBox)
								.addComponent(addArgument)
								.addComponent(saveArgument)
								.addComponent(cancelArgument)
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(argPrefix).addComponent(prefixTf)
								.addComponent(editPrefix)
								.addComponent(savePrefix))
				.addGroup(
						layout.createParallelGroup(
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(newAlgorithm).addComponent(save)
								.addComponent(delete).addComponent(cancel))
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(prev)) {
			if (algorithmList != null && totalAlgorithms > 0) {
				...
				updateArguments(algorithmList.get(currentAlgNum)); // update
																	// list or
																	// arguments
			}
		...
		// return to previous panel
			...
			File f = FileSetUp.getFilepath("Select filepath to Algorithm",
					false);
			if (f != null) {
			...
			descriptionTf.setEditable(false);
			if (isNewAlgorithm) {
				tempParameters.get(currentArgNum).setDescription(
						descriptionTf.getText());
			} else {
				algorithmList.get(currentAlgNum).getArguments()
						.get(currentArgNum)
			...
			if (isNewAlgorithm) {
				...
				algorithmList.get(currentAlgNum).getArguments()
						.get(currentArgNum).setPrefix(prefixTf.getText());
				Main.getResourceHandler().writeAlgorithms();
			}
			...
			defaultValueTf.setEditable(false);
			if (isNewAlgorithm) {
				tempParameters.get(currentArgNum).setValue(
						defaultValueTf.getText());
			} else {
				algorithmList.get(currentAlgNum).getArguments()
						.get(currentArgNum).setValue(defaultValueTf.getText());
				Main.getResourceHandler().writeAlgorithms();
			}
		}
			...
			if (totalAlgorithms > 0) {
				if (argumentBox.getItemCount() > 1) {
					if (currentArgNum < (argumentBox.getItemCount() - 1)) {
						algorithmList.get(currentAlgNum).swapParameter(
			...
			if (a.getArguments().size() > 0) {
				if (argumentBox == null) {
					...
					for (int i = 0; i < arguments.length; i++) {
						arguments[i] = ((Parameter) a.getArguments().get(i))
								.getName();
					}
					...
					for (int i = 0; i < a.getArguments().size(); i++) {
						argumentBox.addItem(((Parameter) a.getArguments()
								.get(i)).getName());
					}
				}
	...
	private void updateArgumentDetails() {
		...
		if (!isNewAlgorithm) {
			descriptionTf.setText(((Parameter) algorithmList.get(currentAlgNum)
					.getArguments().get(argumentBox.getSelectedIndex()))
					.getDescription());
			prefixTf.setText(((Parameter) algorithmList.get(currentAlgNum)
					.getArguments().get(argumentBox.getSelectedIndex()))
					.getPrefix());
			defaultValueTf.setText(((Parameter) algorithmList
					.get(currentAlgNum).getArguments()
					.get(argumentBox.getSelectedIndex())).getValue());

		} else {
			if (tempAlgorithm.getArguments().size() > 0) {
				descriptionTf.setText(tempAlgorithm.getArguments()
						.get(argumentBox.getSelectedIndex()).getDescription());
				prefixTf.setText(tempAlgorithm.getArguments()
						.get(argumentBox.getSelectedIndex()).getPrefix());
				defaultValueTf.setText(tempAlgorithm.getArguments()
						.get(argumentBox.getSelectedIndex()).getValue());
			}
		}
		// bit of a long line here -> basically get the parameters from the
		// current algorithm chosen (indexed) -> get the description from the
	...
	private void save() {
		if (nameTf.getText().length() > 0 && filepathTf.getText().length() > 0) {
			...
			tempAlgorithm.finalise(nameTf.getText(), distBox.isSelected(),
					filepathTf.getText());
	...
	private void saveArgument() {
		...
		if (argumentTf.getText().length() > 0) {
			Parameter newParameter = new Parameter(argumentTf.getText(),
					prefixTf.getText(), defaultValueTf.getText(),
					descriptionTf.getText());

			if (isNewAlgorithm) {
				tempAlgorithm.addArgument(newParameter);
				// update argument box?
				System.out.println("tempalgorithms now has "
						+ tempAlgorithm.getArguments().size());
	...
	private void display(int index) {
		...
		algorithmIDTf.setText(String.valueOf(algorithmList.get(index)
				.getAlgorithmID()));
